...
import { updatePieza } from "../../components/api/adress";
 ...
 */
export default function PiezaUpdateForm({ passPieza, afterSubmit }) {
  ...
  useEffect(() => {
    setId(passPieza.id);
    nombre.onChange({ target: { value: passPieza.nombre } });
    serie.onChange({ target: { value: passPieza.serie } });
    precio.onChange({ target: { value: passPieza.precio } });
  }, []);
